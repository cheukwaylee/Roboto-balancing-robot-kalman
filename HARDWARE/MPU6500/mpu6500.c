...
#define GYRO_SEN 7510.4f//1880.0f 
#define MAG_SEN 0.3f
	...
	if(getDeviceID())
		{
			...
			delay_ms(10);
			MPU6500WriteSPI(MPU6500_SMPLRT_DIV, 0x00);       //陀螺仪采样率，0x00(1kHz) 
			delay_ms(10);
			MPU6500WriteSPI(MPU6500_CONFIG, 0x02);
			delay_ms(10);
			MPU6500WriteSPI(MPU6500_INT_PIN_CFG,0x02);
			delay_ms(10);
			MPU6500WriteSPI(MPU6500_GYRO_CONFIG,0x00);   //设置陀螺仪满量程	±250dps	
			delay_ms(10);
			MPU6500WriteSPI(MPU6500_ACCEL_CONFIG,0x00); //设置加速度计满量程	±2g
			delay_ms(10);
...
MPU6500ReadSPI(ACCEL_XOUT_H,buf,14);
	...
	mpu6500_real_data.Temp=mpu6500_raw_data.Temp/333.87 + 21;
...
//	if (abs(mpu6500_raw_data.Gyro_X)<60){
//	mpu6500_real_data.Gyro_Z=0;
//	}else{

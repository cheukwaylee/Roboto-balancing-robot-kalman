...
void Friction_Moni_normal(void)
{
	...
	if (pre_friction_key==2 && RC_Ctl.rc.s1==3 && frictionState == Friction_OFF){
		frictionState = Friction_ON;
	}
	else if(pre_friction_key==2 && RC_Ctl.rc.s1==3 &&frictionState == Friction_ON){
		frictionState = Friction_OFF;
	}
	
	if (frictionState == Friction_ON){
		BurstMove_Friction(100);
	}else{
		BurstMove_Friction(0);
	}
	

	if (frictionState == Friction_ON && RC_Ctl.rc.s1==1){
		BurstMove(-2000);
	}
...
void BurstMove_Friction(float SetSpeed)
{
	...
	PIDOut_Friction2=PID_ControllerDriver(&FrictionPID1.Speed,SetSpeed,NowSpeed_206);
}
...
void PidShootInit(void)
{
	...
	PID_ControllerInit(&FrictionPID1.Speed,50,50,9800,0.002);//50,50,9800,0.002
	PID_SetGains(&FrictionPID1.Speed,shoot_sp,shoot_si,shoot_sd);
	
	PID_ControllerInit(&FrictionPID2.Speed,50,50,9800,0.002);//50,50,9800,0.002
	PID_SetGains(&FrictionPID2.Speed,shoot_sp,shoot_si,shoot_sd);
}
